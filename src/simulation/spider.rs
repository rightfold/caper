...
pub fn simulate<R: Rng>(spiders: &mut SpiderSet, rng: &mut R, dt: f32) {
    simulate_movements(spiders, dt);
    simulate_rotations(spiders, dt);
    simulate_action_changes(spiders, rng, dt);
}

fn simulate_movements(spiders: &mut SpiderSet, dt: f32) {
    ...
    for (position, (angle, action)) in positions.zip(angles.zip(actions)) {
        *position = simulate_movement(dt, *position, *angle, action);
    }
}
...
fn simulate_rotations(spiders: &mut SpiderSet, dt: f32) {
    ...
    for (angle, action) in angles.zip(actions) {
        *angle = simulate_rotation(dt, *angle, action);
    }
}
...
fn simulate_action_changes<R: Rng>(spiders: &mut SpiderSet, rng: &mut R, dt: f32) {
    spiders.state.since_action_change += dt;
    while spiders.state.since_action_change > ACTION_CHANGE_INTERVAL {
        spiders.state.since_action_change -= ACTION_CHANGE_INTERVAL;
        for action in monster_field!(spiders, mut actions).iter_mut() {
            *action = simulate_action_change(rng, action);
        }
    }
}
