...
fn simulate_target_changes<R: Rng>(spiders: &mut SpiderSet, rng: &mut R, dt: f32) {
    let targets = soa_array!(spiders, mut targets);
    let positions = soa_array!(spiders, positions);
    let since_target_changes = &mut spiders.scalars.since_target_changes;
...
fn simulate_velocities(spiders: &mut SpiderSet, dt: f32) {
    let velocities = soa_array!(spiders, mut velocities).iter_mut();
    let positions  = soa_array!(spiders, mut positions).iter_mut();
...
fn simulate_wanderings(spiders: &mut SpiderSet, dt: f32) {
    let velocities = soa_array!(spiders, velocities).iter();
    let targets    = soa_array!(spiders, mut targets).iter_mut();
    let positions  = soa_array!(spiders, mut positions).iter_mut();
    let angles     = soa_array!(spiders, mut angles).iter_mut();
